...
- Ne PAS réécrire les formules de dégâts. Le moteur de calcul de Showdown est publié en open source sous licence MIT (libre d'utilisation) sous le nom @smogon/calc (https://github.com/smogon/damage-calc). Il gère le mode Champions (génération 0 : points de stat, pool, objets, talents et puissances propres à Champions) et toutes les subtilités des dégâts. On l'utilise comme "moteur" et on construit notre propre interface par-dessus. Si une mécanique propre à Champions n'est pas couverte, on l'ajoute par-dessus, en le documentant ici.
- Depuis la v1.17.0 (10 septembre 2026), le moteur est construit depuis la branche master du GitHub de Showdown (scripts/build-engine.sh, archive vendor/smogon-calc.tgz) et non plus pris sur npm : le paquet npm n'a pas été publié depuis mars 2026, alors que le GitHub reçoit les nouveautés de chaque régulation en quelques heures.
...
- Correction du 10 septembre 2026 : la Téracristallisation n'est PAS légale en régulation M-C, seule la Méga-Évolution est jouable (champdex.com, guide Terastallization ; la Téra est annoncée pour plus tard). Le calc Showdown en mode Champions n'a pas de Téra. Le réglage Téra de Calcritique reste disponible (par défaut "Pas de Téra").
- Régulation M-C (mise à jour 1.2.0 du jeu, 9 septembre 2026, jusqu'au 2 décembre 2026, saison classée M-6) : 35 ajouts (Grodoudou, Persian et Persian d'Alola, Canarticho, M. Mime, Avaltout, Méga-Absol Z, Drattak et Méga-Drattak, Méga-Carchacrok Z, Méga-Lucario Z, Chevroum, Sarmuraï et Méga-Sarmuraï, Gorythmic, Pyrobut, Lézargus, Roublenard, Salarsen (2 formes), Krakos, Berserkatt, Palarticho, Wattapik, Wimessir (2 formes), Pohmarmotte, Olivini, Tapatoès (4 plumages), Dogrino, Glaivodo et Méga-Glaivodo), 18 objets (dont Ballon, Casque Brut, Carton Rouge, Bouton Fuite, les 4 Graines de terrain, Champ'Duit, Poireau, Joyau Normal et les nouvelles Méga-Gemmes), 15 attaques et 15 talents (dont Garde Aura de Méga-Lucario Z). Sources : pokemon-zone.com, serebii.net.
...
- Attention (constaté le 10 septembre 2026) : le dossier personnel de la machine virtuelle est propre à chaque session Claude. À chaque nouvelle session, $HOME/calcritique.git, `cgit` et `gh` n'existent plus : les recréer (`git clone --bare https://github.com/Meiynas/calcritique.git $HOME/calcritique.git`, script cgit = `git --git-dir=$HOME/calcritique.git --work-tree=$HOME/mnt/Cal "$@"`, `cgit reset` pour remplir l'index, gh téléchargé depuis les Releases de cli/cli). La connexion GitHub se refait par le "device flow" : l'humain ouvre https://github.com/login/device et tape un code à 8 caractères (portées repo et workflow ; un processus lancé en arrière-plan meurt à la fin de chaque commande, donc on interroge https://github.com/login/oauth/access_token commande par commande avec l'identifiant client de GitHub CLI).
- Le dossier Cal garde une copie inutile de scripts/build-learnsets.py (retiré du dépôt en v1.17.0, suppression impossible depuis la machine virtuelle) et l'archive de transfert calcritique-transfert.tgz : les mettre dans $HOME/calcritique.git/info/exclude quand on recrée le dépôt, pour ne pas les réajouter.
- Réseau (10 septembre 2026) : depuis l'espace de travail cloud, github.com (git clone), raw.githubusercontent.com et npm sont accessibles, mais pas l'API GitHub, pokeapi.co, championsbattledata.com ni meiynas.github.io. Depuis la machine de l'humain : github.com, api.github.com, npm oui ; pokeapi.co et championsbattledata.com non. Le robot GitHub Actions a accès à tout.
...
Ajouté en v1.17.0 (10 septembre 2026, demande de l'humain : "de nouveaux Pokémon ont été ajoutés et rien n'est à jour") :
- Cause : le robot ne mettait à jour que les statistiques d'usage ; le pool légal (learnsets.json, figé le 6 septembre depuis PokéAPI), les noms, descriptions et icônes n'étaient régénérés qu'à la main, et `npm update @smogon/calc` ne pouvait rien apporter (paquet npm 0.11.0 de mars 2026). Le paquet npm connaissait bien les nouvelles Méga, mais avec les données de Légendes Z-A (mauvais talents : Méga-Carchacrok Z en Force Sable au lieu de Lévitation, Méga-Lucario Z en Adaptabilité au lieu de Garde Aura, dégâts faux).
- Moteur : scripts/build-engine.sh clone smogon/damage-calc (master), compile le dossier calc/ (tsc), et range le résultat dans vendor/smogon-calc.tgz (archive reproductible : même moteur = même fichier ; vendor/smogon-calc.json garde le commit et l'empreinte du dossier calc/, rien n'est reconstruit si elle n'a pas changé). package.json : "@smogon/calc": "file:vendor/smogon-calc.tgz". .gitignore : exception !vendor/*.tgz.
- Mode Champions : src/lib/gen.ts exporte `gen = Generations.get(0)` (génération 0 = Pokémon Champions), utilisé par engine, usage, switch et la table des types. Les SP sont passés directement au moteur (plus de conversion en EV). Gains : puissances propres à Champions (Tranche 80, Acide Malique 90, Charge Glaive...), talents et formules de Champions, listes d'objets / attaques / talents du jeu (166 objets, 525 attaques, 215 talents, 359 Pokémon et formes).
- Objet ou talent absent de Champions (vieux set, import Showdown) : ignoré par le moteur (engine.isChampionsItem), sinon il plantait (item.megaStone) ; ⚠ à côté de l'objet dans la fiche.
- Pool légal et attaques apprenables : scripts/build-showdown-data.mts (lancé avec tsx) lit les données de Pokémon Showdown (data/pokedex.ts, data/moves.ts, data/mods/champions/learnsets.ts, formats-data.ts, moves.ts) : Pokémon disponibles en Champions sans fabuleux ni légendaire restreint (règles "Flat Rules" du VGC Showdown), formes sans entrée propre rattachées à leur espèce (Palafin Héros, Morpeko Affamé, Mistigrix femelle...), attaques de l'espèce, de sa forme de base et de ses pré-évolutions. Écrit aussi calcnames.json et showdown.json (précision et priorité avec les changements Champions : Pince-Masse 95, Pluie de Pièces 95...). scripts/build-learnsets.py (PokéAPI) est retiré.
- scripts/fetch-sources.sh télécharge les 13 CSV de PokéAPI utiles (depuis raw.githubusercontent.com) et un clone partiel de Pokémon Showdown. build-names.py (alias de formes Tapatoès / Famignol, noms manuels de scripts/names-fr.json : 39 Méga-Gemmes de Légendes Z-A et de Méga-Dimension, vérifiés sur Poképédia et jeuxvideo.com), build-extra.py (précision / priorité / talents depuis showdown.json, identifiant PokéAPI pour les icônes), build-movedesc.py (correspondance tolérante aux apostrophes : Bouclier Royal), build-sprites.py (télécharge seulement les icônes manquantes, garde les autres telles quelles). scripts/refresh-data.sh enchaîne tout.
- Robot (.github/workflows/data-refresh.yml) : toutes les 6 heures (cron 17 */6 * * *) ou à la demande : moteur, sources, pool / noms / icônes, statistiques d'usage (chaque étape peut échouer sans bloquer les autres), puis si src/data, vendor ou package*.json ont changé : installation propre (npm ci), tests, construction du site, version +1, commit "Données du ... : moteur Showdown, Pokémon et attaques de la régulation, noms, talents, icônes, statistiques d'usage" selon ce qui a changé, étiquette, installeur et site.
- Protection : Transperceuse (Méga-Minotaupe) passe Abri sur les attaques de contact comme Poing Invisible. Eelevate (Méga-Ohmassacre) compte comme Lévitation pour les pièges, les terrains et la table des types.
- Fenêtre de choix des Pokémon : colonne des noms élargie (Méga-Carchacrok Z n'était pas lisible en entier).
- Tests adaptés au pool Champions (Prismillon au lieu de Lucanon, Carchacrok au lieu de Démétéros, Méga-Minotaupe et Transperceuse au lieu de Shifours et Poing Invisible, Bazoucan avec Multi-Coups au lieu de Crustabri avec Dé Pipé, objets absents de Champions) et 3 nouveaux tests (nouveautés M-C, objet absent du jeu, set automatique avec pierre Méga Z). 24 tests.
...
- Les noms français des nouvelles formes Méga sont construits automatiquement ("Méga-" + nom + suffixe, ex. Méga-Carchacrok Z). Les objets et talents inconnus de PokéAPI restent en anglais tant qu'ils ne sont pas ajoutés à scripts/names-fr.json (le 10 septembre 2026 : Aura Guard, Eelevate, Fire Mane).
- Tous les fichiers de src/data sont régénérés automatiquement par le robot (scripts/refresh-data.sh et scripts/fetch-usage.mjs).
